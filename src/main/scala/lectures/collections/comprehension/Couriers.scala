...
object CouriersWithComprehension extends App {
  ...
  def serveAddresses(addresses: List[Address], couriers: List[Courier]) = {
    val addressesCount = addresses.length
    
    val served = couriers.map(c => if (traffic().degree > 5) c.canServe else 0).sum
    (0 until served).toList.flatMap(i => 
      if (i < addressesCount) List(addresses(i)) else List()
    )
  }
